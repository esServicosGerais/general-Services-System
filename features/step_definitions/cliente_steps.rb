...
Given('o cliente de cpf {string} esta cadastrado') do |cpf|
  fill_in 'cliente[nome]', :with => 'Laura'
  fill_in 'cliente[cpf]', :with => cpf
  fill_in 'cliente[telefone]', :with => '(87) 98145-4587'
  fill_in 'cliente[email]', :with => 'laura@hotmail.com'
  fill_in 'cliente[dataNascimento]', :with => '2000-05-28'
  fill_in 'cliente[cidade]', :with => 'Garanhuns'
  fill_in 'cliente[bairro]', :with => 'Centro'
  fill_in 'cliente[logradouro]', :with => 'Santo Antonio'
  fill_in 'cliente[cep]', :with => '55294-272'
  fill_in 'cliente[complemento]', :with => 'Apartamento'
  click_button 'Create Cliente'
  expect(page).to have_content(cpf)
